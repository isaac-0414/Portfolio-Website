import { getW, getH, setW, setH, getO, setO, Orientation  } from "../global/window_size";
...
import { console2AutoType, getConsole2IntervalId, getConsole2TimeoutId } from "../util/console2";
import { loadSkills } from "../components/skills";
import { loadProjects, projectsPageNumCol } from "../components/projects";
...
function handleResize () {
    ...
    if (getW() === window.innerWidth && getH() === window.innerHeight)
        return;
    ...
    if (getW() <= 768 || getO() === Orientation.portrait) {
        ...
        if (getCurrentPage() !== Page.contact && getCurrentPage() !== Page.projects) {
            console_1.classList.remove("middle");
            console_2.classList.remove("middle", "hidden");
        }
        menu.classList.remove("inactive");
        if (getCurrentPage() === Page.about) {
            if (getConsole2IntervalId() === undefined && getConsole2TimeoutId() === undefined) {
                console2AutoType();
            }
        }
        if (getCurrentPage() === Page.skills) {
    ...
    if (getCurrentPage() === Page.projects) {
        if((getW() <= 480 && projectsPageNumCol() !== 1) || (getW() > 480 && getW() <= 768 && projectsPageNumCol() !== 2) || (getW() > 768 && getW() <= 1400 && projectsPageNumCol() !== 3) || (getW() > 1400 && projectsPageNumCol() !== 4)) {
            loadProjects();
        }
    }
}
